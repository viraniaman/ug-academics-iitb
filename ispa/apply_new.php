...
.ui-jqgrid tr.jqgrow td {
    ...
    <script>
    function warning(){
    	alert("Note: You cannot leave the project in middle. If you do that then you cannnot write the same project in your resume.");
    }
			var count_1=0;
			var count_2=0;
			var count_3=0;
			...
			var preference_4="preference-4";

		function mySubmit(id){	
			alert("tp");
			$("#toolbar .preference").each(function(i){
				alert(id);
				val = $(this).val();
				...
				if (count_1>1){
				alert("Preference 1 is selected more than once. Please fill all the choices again.");
				err=1;
				}
			/*else if (count_2<=0){
			 ...
			 err=1;
				...
				alert("Preference 2 is selected more than once. Please fill all the choices again.");
				err=1;
				}
			/*else if (count_3<=0){
			 ...
			 err=1;
				...
				alert("Preference 3 is selected more than once. Please fill all the choices again.");
				err=1;
				}
			else if (count_4>1){
				alert("Preference 4 is selected more than once. Please fill all the choices again.");
				err=1;
				}
		}
		...
		$("#check").click(function(){
		 ...
		 if (err==0){
			 $.ajax({
				type: 'POST',
				url: 'savedata.php',
				data : "user="+username+"&department="+department+"&preference_1="+preference_1+"&preference_2="+preference_2+"&preference_3="+preference_3+"&preference_4="+preference_4,
...
/////////////////////
	...
	function gridSearch2(department)
    {
       ...
       jQuery("#toolbar2").jqGrid({
			...
			width: 700,
			autowidth: true,
			colNames:['Prof. Name','Interests','Eligibility Criteria','Preference'],
			colModel:[
...
<div id="table2">
    ...
    echo "<select name='department' id='choose-dep2'>";
    foreach ($alldepartments as $key=>$value){
		if ($value['value']!=$mydepartment){
	...
	<td>	<input style="margin-left:50%;" type="button" class="btn btn-primary btn-large" onclick="warning();" id="check" value="Submit"> </td>
