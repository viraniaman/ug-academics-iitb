...
//populateDBRandom();
$result = mysql_query("SELECT COUNT(*) AS count FROM projects_2013 WHERE department=$department");
...
if ($start<0) $start = 0;
$SQL = "SELECT id, prof_name, project_title,project_description,eligibility_criteria FROM projects_2013 WHERE department='$department' ORDER BY 'id' ";
...
while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
    ...
    $responce->rows[$i]['cell']=array($row['prof_name'],$row['project_title'],$row['project_description'],$row['eligibility_criteria'],'4');
}
else{
